...
use egui::{Align, Button, Layout, RichText, Spinner, Ui, vec2};
...
const WALLET_DISCOVERY_IN_PROGRESS_MSG: &str = "Wallet discovery in progress, do not close the app yet!";
   ...
   ui.horizontal(|ui| {
      ui.vertical(|ui| {
         ui.horizontal(|ui| {
            ...
            gui.chain_selection.show(ctx.clone(), theme, icons.clone(), ui);
         });
      ...
      if ctx.wallet_discovery_in_progress() {
         ui.with_layout(Layout::right_to_left(Align::Min), |ui| {
            frame.show(ui, |ui| {
               ui.label(RichText::new(WALLET_DISCOVERY_IN_PROGRESS_MSG).size(theme.text_sizes.normal));
               ui.add_space(10.0);
               ui.add(Spinner::new().size(20.0));
            });
         });
