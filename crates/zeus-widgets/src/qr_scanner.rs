   ...
   pub fn show(&mut self, ctx: &egui::Context) {
      ...
      ctx.show_viewport_deferred(
         ...
         move |ctx, class| {
            ...
            match capture_and_decode(
               capture_size.load(Ordering::Relaxed),
               &monitor_clone,
            ) {
               Ok(res) => {
            ...
            let (mouse_x_px, mouse_y_px) =
               enigo::Enigo::new(&enigo::Settings::default()).unwrap().location().unwrap();
            ...
            egui::Area::new("qr_help".into()).fixed_pos(help_pos).show(&ctx, |ui| {
               ...
               frame.show(ui, |ui| {
                  ...
                  let text = RichText::new(
                     "Press + or - to increase or decrease the size of the captured area.",
                  )
                  .size(14.0);
                  ui.label(text);
