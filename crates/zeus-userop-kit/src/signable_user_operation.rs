...
impl SignableUserOperation {
   ...
   pub async fn sign(
      ...
      user_op.signature = signer.sign_hash(&self.signing_hash()).await?.as_bytes().into();
...
mod tests {
   ...
   async fn test_sign() {
      ...
      let recovered = signature.recover_address_from_prehash(&signing_hash).unwrap();
      assert_eq!(
         recovered,
         signer.address(),
