...
use crate::{
    ...
    indexer::syncer::{self, normalize_tree_position::normalize_tree_position, SyncEvent},
};
    ...
    fn from(value: Commitment) -> Self {
        ...
        match value.kind {
            ...
            CommitmentKind::ShieldCommitment {
                ...
                // Note: the bundle structure for shield in subsquid is:
                // encrypted_bundle[0] = iv(16) + tag(16) for the note?
                // For consistency with zeus Shield, we map similarly to kohaku.
                let mut data = Vec::new();
                if encrypted_bundle.len() > 1 {
                    data.push(encrypted_bundle[1][..].to_vec());
                }
                SyncEvent::Shield(
                    syncer::Shield {
                        tree_number,
...
impl From<Nullified> for SyncEvent {
    fn from(value: Nullified) -> Self {
        SyncEvent::Nullified(
            syncer::Nullified {
                tree_number: value.tree_number,
                nullifier: FixedBytes::from_slice(&value.nullifier.to_be_bytes::<32>()),
            },
            value.block_number,
        )
    }
