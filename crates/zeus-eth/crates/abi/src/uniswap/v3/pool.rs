    ...
    where P: Provider<(), N> + Clone + 'static, N: Network
{
    ...
    let tick: i32 = slot0._1.to_string().parse().context("Failed to parse tick")?;
    Ok((U256::from(slot0._0), tick, slot0._2, slot0._3, slot0._4, slot0._5, slot0._6))
}
    ...
    where P: Provider<(), N> + Clone + 'static, N: Network
{
    ...
    let tick_cumulative_inside: i64 = snapshot_cumulatives_inside.tickCumulativeInside
        .to_string()
        .parse()
    ...
    where P: Provider<(), N> + Clone + 'static, N: Network
{
    ...
    let contract = IUniswapV3Pool::new(pool_address, client);
    let tick_bitmap = contract.tickBitmap(word_position).block(block).call().await?;
    Ok(tick_bitmap._0)
}
    ...
    where P: Provider<(), N> + Clone + 'static, N: Network
{
    ...
    let tick_spacing = tick_spacing.parse::<i32>().context("Failed to parse tick spacing")?;
    Ok(tick_spacing)
}
