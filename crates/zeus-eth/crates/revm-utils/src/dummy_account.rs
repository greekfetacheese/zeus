...
use super::fork_db::ForkFactory;
use super::new_evm;
use super::simulate::erc20_balance;
...
impl DummyAccount {
   ...
   pub fn find_balance_slot<P>(
      &self,
      fork_factory: &mut ForkFactory<P>,
      token: Address,
      amount: U256,
      ...
      P: Provider<(), Ethereum> + Clone + 'static + Unpin,
   {
      ...
      for slot in slot_range {
         ...
         self.insert_with_slot(&mut cloned_fork_factory, slot, token, amount)?;

         let db = cloned_fork_factory.new_sandbox_fork();
         let mut evm = new_evm(1, None, db);
      ...
      P: Provider<(), Ethereum> + Clone + 'static + Unpin,
   {
      ...
      if let Some(slot) = slot {
         self.insert_with_slot(fork_factory, slot, token, amount)
      } else {
         Err(anyhow::anyhow!(
      ...
      P: Provider<(), Ethereum> + Clone + 'static + Unpin,
   {
      ...
      fork_factory.insert_account_info(address, account_info);
